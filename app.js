...
/* =====================================================
   FINAL ACCOUNT PROFILE RECOVERY
===================================================== */

/* =====================================================
   FINAL ACCOUNT PROFILE RECOVERY
   Uses the authenticated Supabase session directly and
   loads the customer row by auth_user_id. This is kept
   independent of the other page modules so the account
   page cannot be left with placeholder values.
===================================================== */
if (document.getElementById("profileName")) {
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 15 && !authUser; attempt++) {
        const { data, error } = await client.auth.getSession();
        authSession = data?.session || null;
        authUser = authSession?.user || null;
        if (!authUser) await new Promise(resolve => setTimeout(resolve, 250));
      }
      ...
      /* Direct table lookup is permitted by the existing own-profile RLS policy. */
      ...
      if (!direct.error && direct.data) {
        ...
        profileError = direct.error || null;
      }
      ...
      if (!profile) {
        ...
        if (!rpc.error) {
          ...
          profileError = rpc.error;
        }
      }

      const meta = authUser.user_metadata || {};
      const value = (field, fallback = "") => profile?.[field] ?? meta[field] ?? fallback;

      const name = value("name", "Customer");
      const email = profile?.email || authUser.email || "—";
      const phone = value("phone", "Not added");
      const address = value("address", "—");
      const city = value("city", "");
      const district = value("district", "");
      const cityDistrict = [city, district].filter(Boolean).join(", ") || "—";

      if (byId("customerName")) byId("customerName").textContent = name;
      if (byId("profileName")) byId("profileName").textContent = name || "—";
      if (byId("profileEmail")) byId("profileEmail").textContent = email;
      if (byId("profilePhone")) byId("profilePhone").textContent = phone;
      if (byId("profileAddress")) byId("profileAddress").textContent = address;
      if (byId("profileCity")) byId("profileCity").textContent = cityDistrict;

      if (byId("editName")) byId("editName").value = profile?.name || meta.name || "";
      if (byId("editEmail")) byId("editEmail").value = email === "—" ? "" : email;
      if (byId("editPhone")) byId("editPhone").value = profile?.phone || meta.phone || "";
      if (byId("editAddress")) byId("editAddress").value = profile?.address || meta.address || "";
      if (byId("editCity")) byId("editCity").value = profile?.city || meta.city || "";
      if (byId("editDistrict")) byId("editDistrict").value = profile?.district || meta.district || "";

      if (profile && byId("profileMessage")) {
        byId("profileMessage").textContent = "";
        byId("profileMessage").className = "message";
      }

      if (!profile && profileError && byId("profileMessage")) {
        byId("profileMessage").textContent = "Unable to load customer profile: " + profileError.message;
        byId("profileMessage").className = "message error";
      }
    } catch (error) {
      console.error("Account profile recovery failed:", error);
      const message = byId("profileMessage");
      if (message) {
        message.textContent = "Unable to load your account details. Please refresh and try again.";

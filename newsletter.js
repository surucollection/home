(function () {
  ...
  if (!form || !window.supabase || !window.SURU_SUPABASE_URL || !window.SURU_SUPABASE_KEY) return;
  ...
  form.addEventListener('submit', async function (event) {
    ...
    try {
      ...
      if (error) {
        ...
        status.textContent = 'Thank you! You are now subscribed to Suru Collection updates.';
        status.className = 'newsletter-status success';
        form.reset();
      }
    } catch (error) {
